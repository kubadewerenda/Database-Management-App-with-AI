...
export default class UserService {
  ...
  public _verifyToken(token: string): {
    sub: number;
    iat: number;
    exp: number;
  } {
  ...
  private async sendVerificationEmail(user: User): Promise<void> {
    const appUrl = (
      process.env.FRONTEND_URL_BASE || "http://localhost:5173"
    ).replace(/\/$/, "");
  ...
  public async updateUser(
    ...
    { username, email, currentPassword, newPassword }: UpdateMeD
  ) {
    ...
    if (currentPassword || newPassword) {
      ...
      const isPassOk = await this._verifyPassword(
        currentPassword,
        user.passwordHash
      );
      if (!isPassOk)
      ...
      const isDifferent = await this._verifyPassword(
        newPassword,
        user.passwordHash
      );
      if (isDifferent)
        throw new BadRequestException(
          "New password has to be different than the last one.",
          ErrorCodeEnum.VALIDATION_ERROR
        );
  ...
  public async loginWithGoogle(code: string) {
    ...
    if (!email || !sub) {
      throw new BadRequestException("Google account must have email and sub.");
    }
